...
| Inspect adaptive allocation | `ccr optimizer plan/status/report --run RUN_ID --json` | read-only; no provider calls |
| Reserve adaptive work | `ccr optimizer step --run RUN_ID --apply` | atomic trial, task, resource reservation, and audit outbox |
| Execute an optimizer trial | `ccr optimizer run --run ... --trial ... --worker ... --fencing-token ... --execute --config ...` | existing operator approval and exact bound config required |
